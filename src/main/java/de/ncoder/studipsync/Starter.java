...
public class Starter {
	...
	public static void main(String[] args) {
		...
		try {
			...
			log.error("Syncer didn't finish properly", e);
		} finally {
			if (displayHelp) {
	...
	public static Syncer createSyncer(StarterOptions options) throws IOException {
		...
		final JsoupStudipAdapter browser = new JsoupStudipAdapter(options.getUIAdapter(), options.getCookiesPath(), options.getTimeoutMs());
		final File page = new File("page.html");
		browser.addNavigationListener(new JsoupStudipAdapter.NavigationListener() {
			@Override
			public void navigated(URL url) {
				try (Writer w = new FileWriter(page, false)) {
					w.append(browser.getDocument().html());
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
